...
var ExchangeWebService_2 = require("../../../src/js/ExchangeWebService");
...
chai.should();
describe("Calendar Operation tests", function () {
    ...
    it.skip("find appointments and load extended property ", function (done) {
    ...
    it.skip("Create extended property ", function (done) {
        mockXhr.requestXml = [MockXHRData_1.MockXHRData.Operations.ItemOperations.FindItemRequest1ItemView];
        mockXhr.responseXml = [MockXHRData_1.MockXHRData.Operations.ItemOperations.FindItemRequest1ItemViewResponse];
        var PR_TRANSPORT_MESSAGE_HEADERS = new ExchangeWebService_1.ExtendedPropertyDefinition(ExchangeWebService_1.MapiPropertyType.String, 0x007D);
        var psPropSet = new ExchangeWebService_1.PropertySet(ExchangeWebService_1.BasePropertySet.IdOnly, [PR_TRANSPORT_MESSAGE_HEADERS]);
    ...
    it("GetUserAvailability handles empty WorkingPeriod DayOfWeek", function (done) {
        mockXhr.responseXml = [MockXHRData_1.MockXHRData.Operations.CalendarOperations.GetUserAvailabilityRequestResponseWithEmptyDayOfWeek];
        exch.XHRApi = mockXhr;
        var timeWindow = new ExchangeWebService_2.TimeWindow(new ExchangeWebService_1.DateTime(new Date()), new ExchangeWebService_1.DateTime(new Date(new Date().getTime() + (60 * 60 * 24 * 1000))));
        var attendees = [new ExchangeWebService_2.AttendeeInfo('test@example.com')];
        exch.GetUserAvailability(attendees, timeWindow, ExchangeWebService_2.AvailabilityData.FreeBusy).then(function (response) {
            chai.assert.equal(response.AttendeesAvailability.Count, 1);
            chai.assert.deepEqual(response.AttendeesAvailability.Responses[0].WorkingHours.DaysOfTheWeek, []);
            done();
        });
    });
});
